...
- Stopped copying analyzer/source-generator DLLs from `Editor/Analyzers/` into `Assets/Plugins/Editor/WallstopStudios.DxMessaging/` on editor load. The package's analyzer DLLs in `Editor/Analyzers/` now ship with the `RoslynAnalyzer` asset label so Unity picks them up directly, and removing the auto-copy resolves a Unity 2021 `PrecompiledAssemblyException: Multiple precompiled assemblies with the same name WallstopStudios.DxMessaging.SourceGenerators.dll` that broke playmode/standalone tests when both the package copy and the auto-copied duplicate were imported. The package also ships a one-shot cleanup pass (`SetupCscRsp.CleanUpLegacyAnalyzerCopyPath`) that runs on `EditorApplication.delayCall` and deletes the legacy duplicate analyzer DLLs (plus the now-empty `Assets/Plugins/Editor/WallstopStudios.DxMessaging/` directory) when it can. **First-launch caveat:** because Unity 2021 throws `PrecompiledAssemblyException` during asset import (before `[InitializeOnLoad]` scripts run), the cleanup cannot fire on the FIRST editor launch after upgrading a project that still has the legacy duplicates. On that first launch the user needs to manually delete `Assets/Plugins/Editor/WallstopStudios.DxMessaging/` once. On every subsequent launch the cleanup pass keeps that directory clean -- and on Unity 2022+ (where the duplicate doesn't fatal-error) the cleanup runs unattended even on the first launch. The cleanup is gated by a marker file at `Library/.dxm-cleanup-done` and runs at most once per project; if a user downgrades and re-upgrades (uncommon), deleting that marker re-triggers the cleanup on the next editor load.
