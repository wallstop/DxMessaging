...
supported Unity version and posts the refreshed numbers as a non-blocking comment
on the pull request; the autogenerated dispatch-throughput table above is then
refreshed by a commit to master after the PR merges. The numbers therefore track
the actual measured
