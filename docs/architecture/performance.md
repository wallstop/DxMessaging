...
The table below is regenerated by the Unity Benchmarks workflow from the latest
Unity version's scheduled benchmark run and is opened as a docs pull request
when the numbers move outside tolerance. Do not edit it by hand.
...
The documented dispatch-throughput numbers are maintained entirely by CI; you do
not hand-capture them in the PR description. When a pull request targets `master`
or `main`, the Performance Numbers workflow (`.github/workflows/perf-numbers.yml`)
re-runs the benchmark suite on the pinned self-hosted runner at the latest
supported Unity version and commits the refreshed numbers into the autogenerated
dispatch-throughput table above. The numbers therefore track the actual measured
throughput of the branch under review rather than committed state, a manually
pasted table, or specific PR text.

When you change one of the hot paths, review the table the workflow commits onto
your branch:
...
An unexpected throughput drop in the committed table is a regression to
