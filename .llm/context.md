...
- Unity Pro is a single-seat license. All Unity-credential-using jobs (`unity-tests`, `benchmarks`, `release.unity-checks`) share `concurrency.group: unity-pro-license` with `cancel-in-progress: false`, and the matrix jobs MUST set `strategy.max-parallel: 1` so matrix entries do not compete for the license lock and evict each other. The validator (`findMatrixConcurrencyEvictionViolations`) enforces this combination. IL2CPP is no longer a separate job -- it is the `standalone` entry in the `unity-tests.yml` `test-mode` matrix (native game-ci `testMode: standalone`, IL2CPP from ProjectSettings, runtime-only assemblies). Both ELI-MACHINE and DAD-MACHINE are eligible for every Unity job via the uniform `runs-on: [self-hosted, Windows, RAM-64GB]`; cross-workflow serialization comes from the shared group, within-workflow serialization from `max-parallel: 1`. The `fast` label remains on ELI-MACHINE for future opt-in hotfix dispatches but no job requests it today.
