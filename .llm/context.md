...
- Before declaring a task done or pushing, you do not need to remember per-file checks -- the automation runs them. `npm run preflight` inspects exactly what you changed (committed-vs-integration-base + staged + unstaged + untracked), delegates file -> hook selection to pre-commit itself (`--from-ref/--to-ref` + `--files`), self-heals `node_modules` / `pre-commit` first, and runs the relevant lint/spelling/doc/changelog/YAML/policy checks in-loop. A PreToolUse guard runs it (full scope) before your `git push` and blocks on failure; an advisory Stop hook runs it (working-tree scope) when you end a turn. A PostToolUse guard (`scripts/hooks/post-edit-validate-guard.js`) runs the fast validators relevant to each edited file (npm-packaging when you touch `package.json` / `.npmignore` / `*.dll` / `*.meta`; ASCII/prose/code-sample checks for `.md` / `.cs`) so those failures surface at edit time instead of waiting for the push. For full parity on demand run `npm run preflight:pre-push`. The full file-glob -> validator dispatch table (with per-failure remediation, e.g. `W002` guidance), the flags, the Node-direct fallback, and the loud yamllint-skip live in [Change-Aware Preflight](./skills/scripting/change-aware-preflight.md).
