...
- **Use per-extension loops for `git add --renormalize`**: Multi-pattern renormalize commands fail with exit code 128 if any pattern matches no files. Use a loop to process each extension separately with existence checks. Example: `for ext in md json yml; do git ls-files "*.${ext}" "**/*.${ext}" | head -1 | grep -q . && git add --renormalize -- "*.${ext}" "**/*.${ext}"; done`.
