...
- Before declaring a task done or pushing, you do not need to remember per-file checks -- the automation runs them. `npm run preflight` inspects exactly what you changed (committed-vs-integration-base + staged + unstaged + untracked), delegates file -> hook selection to pre-commit itself (`--from-ref/--to-ref` + `--files`), self-heals `node_modules` / `pre-commit` first, and runs the relevant lint/spelling/doc/changelog/YAML/policy checks in-loop. A PreToolUse push guard first runs changed-file cspell over the full change-set, then runs full-scope preflight and blocks on failure; the direct cspell pass prevents already-committed, generated, or shell-written spelling failures from reaching native pre-push when the broader guard preflight is slow. An advisory Stop hook runs preflight in working-tree scope when you end a turn. A PostToolUse guard (`scripts/hooks/post-edit-validate-guard.js`) runs the fast validators relevant to each edited file (file-scoped cspell for `.md` / `.markdown` / `.cs` / `.json` / `.yml` / `.yaml` / `.ps1` / `.js`; npm-packaging when you touch `package.json` / `.npmignore` / `*.dll` / `*.meta`; ASCII/prose/code-sample checks for `.md` / `.cs`) so those failures surface at edit time instead of waiting for the push. For full parity on demand run `npm run preflight:pre-push`. The full file-glob -> validator dispatch table (with per-failure remediation, e.g. `W002` guidance), the flags, the Node-direct fallback, and the loud yamllint-skip live in [Change-Aware Preflight](./skills/scripting/change-aware-preflight.md).
- Native Git hooks are versioned under `scripts/hooks` and installed by `scripts/install-git-hooks.js` through `core.hooksPath`; the `pre-commit` and `pre-push` hooks repair Node tooling, auto-install pinned `pre-commit`, and delegate to the pre-commit framework. The `pre-push` hook delegates the full parity sweep to `scripts/run-prepush-parallel.js`, which runs the mutating hooks (csharpier, dotnet-tool-restore) serially first (so auto-fixers heal the tree before the read-only freshness checks observe it), then the read-only pre-push hooks plus the preflight-only validators concurrently, deduping the subset Jest suites against the full `script-tests` suite and the cspell subsets against the cspell hook. It has the same coverage as `npm run preflight:pre-push` (the byte-for-byte serial parity command kept for CI and on-demand use); coverage equivalence and the dedup proofs are pinned by `scripts/__tests__/run-prepush-parallel.test.js`. The git hook is the last-resort backstop, not the first signal -- see [Native Git Hook Bootstrap](./skills/scripting/native-git-hooks.md).
