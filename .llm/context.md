...
- `scripts/sync-banner-version.sh` prefers open-source PowerShell 7+ (`pwsh`), then legacy `powershell`, then falls back to `node scripts/sync-banner-version.js` so banner sync works on Linux, macOS, and Windows.
