...
- Unity Pro is a single-seat license. Every Unity-credential-using job (`unity-tests`, `benchmarks`, `release.unity-checks`) must run `./.github/actions/validate-unity-license`, then acquire `wallstop-organization-builds` through `Ambiguous-Interactive/ambiguous-organization-build-lock/.github/actions/acquire-build-lock@v1` immediately before `game-ci/unity-test-runner@v4`, then release it with `if: always()` through `release-build-lock@v1`. This lets pre-Unity setup and secret-shape validation split across eligible runners while serializing only the licensed Unity section across repositories. IL2CPP is the `standalone` entry in the `unity-tests.yml` `test-mode` matrix (native game-ci `testMode: standalone`, IL2CPP from ProjectSettings, runtime-only assemblies). Both ELI-MACHINE and DAD-MACHINE are eligible for every Unity job via the uniform `runs-on: [self-hosted, Windows, RAM-64GB]`; the `fast` label remains on ELI-MACHINE for future opt-in hotfix dispatches but no job requests it today.
...
- Enforcement: `scripts/validate-workflows.js` (`npm run validate:workflows`) lints every workflow file for native misuse of the organization lock group, matrix-with-shared-group eviction, unsupported `game-ci/unity-test-runner@v4` inputs, missing organization lock acquire/release wrappers, missing Unity license preflight, and the self-hosted label allowlist. The shape contract test `scripts/__tests__/unity-workflow-shape.test.js` plus `scripts/__tests__/validate-workflows-concurrency-and-labels.test.js` keep the Unity-credential-using jobs honest. The validator scans `.github/workflows/*.yml` only; the `.github/workflows-disabled/*` mirror is intentionally left at the old `${{ github.workflow }}-${{ github.ref }}` group shape and is not policed.
