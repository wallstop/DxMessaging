...
- Before declaring a task done, committing, or pushing, you do not need to remember per-file checks -- the automation runs them. `npm run preflight` inspects exactly what you changed (committed-vs-integration-base + staged + unstaged + untracked), delegates file -> hook selection to pre-commit itself (`--from-ref/--to-ref` + `--files`), self-heals `node_modules` / `pre-commit` first, and runs the relevant lint/spelling/doc/changelog/YAML/policy checks in-loop. A PreToolUse git-boundary guard first runs changed-file cspell over the full change-set, then runs full-scope preflight and blocks likely `git commit` / `git push` commands on failure; commit commands are scoped to the pre-commit stage for speed. The direct cspell pass prevents already-committed, generated, or shell-written spelling failures from reaching native pre-push when the broader guard preflight is slow. An advisory Stop hook runs preflight in working-tree scope when you end a turn. A PostToolUse guard (`scripts/hooks/post-edit-validate-guard.js`) runs the fast validators relevant to each edited file (file-scoped cspell for `.md` / `.markdown` / `.cs` / `.json` / `.yml` / `.yaml` / `.ps1` / `.js`; changelog coverage when you touch likely user-visible `Runtime/` / `Editor/` / `SourceGenerators/` / `Samples~/` files or `CHANGELOG.md`; npm-packaging when you touch `package.json` / `.npmignore` / `*.dll` / `*.meta`; ASCII/prose/code-sample checks for `.md` / `.cs`) so those failures surface at edit time instead of waiting for the native hook. For full parity on demand run `npm run preflight:pre-push`. The full file-glob -> validator dispatch table (with per-failure remediation, e.g. `W002` guidance), the flags, the Node-direct fallback, and the loud yamllint-skip live in [Change-Aware Preflight](./skills/scripting/change-aware-preflight.md).
- Native Git hooks are versioned under `scripts/hooks` and installed by `scripts/install-git-hooks.js` through `core.hooksPath`; only commit/push boundary hooks are required (`pre-commit`, `pre-push`). Do not add post-checkout/post-merge/post-rewrite mutating hooks; ordinary git operations must not dirty the tree or bootstrap pre-commit. The `pre-commit` and `pre-push` hooks repair Node tooling, auto-install pinned `pre-commit`, and delegate to the pre-commit framework. After the PreToolUse commit guard passes it writes a pre-commit stamp covering HEAD, the index tree, and pathspec-limited unstaged/untracked changelog-relevant content so the native pre-commit wrapper can skip the repeated run in under a second only when the validated commit/changelog surface is unchanged. A successful full `npm run preflight:pre-push` writes a pre-push stamp covering HEAD, the Git index file hash, unstaged tracked worktree diff metadata plus changed tracked file bytes, and the empty untracked/unignored path set so native pre-push can skip the repeated exhaustive sweep in under a second only when the validated push surface is unchanged; the stamp writer refuses to write while untracked paths exist. The serial and parallel pre-push runners strip caller-provided `SKIP` before authoritative validation steps and only apply controlled lane-local `SKIP` values, so users cannot skip hooks and still mint a stamp. The `pre-push` hook delegates the full parity sweep to `scripts/run-prepush-parallel.js`, which runs the mutating hooks (csharpier, dotnet-tool-restore) serially first (so auto-fixers heal the tree before the read-only freshness checks observe it), then the read-only pre-push hooks plus the preflight-only validators concurrently, deduping the subset Jest suites against the full `script-tests` suite and the cspell subsets against the cspell hook. It has the same coverage as `npm run preflight:pre-push` (the byte-for-byte serial parity command kept for CI and on-demand use); coverage equivalence and the dedup proofs are pinned by `scripts/__tests__/run-prepush-parallel.test.js`. The git hook is the last-resort backstop, not the first signal -- see [Native Git Hook Bootstrap](./skills/scripting/native-git-hooks.md).
...
- On a `testRunner option was not found` or any `jest-circus` resolution error, run `npm run doctor` and consult [Jest Hook Robustness](./skills/scripting/jest-hook-robustness.md). On Windows, suspect a partial extract; `scripts/lib/node-modules-integrity.js` auto-repairs via `npm ci`. The corrupt isolated managed-Jest cache (a regenerable tmpdir fallback) is auto-cleared by `npm run repair:node-tooling` and the agentic preflight before the native hook fires -- zero manual touch; the doctor reports it as WARN, never a hard fail. When interpolating a path into a `warnFn` / `console.warn` / `console.error` log line in the managed-Jest / integrity-gate scripts, wrap it in `toPosixPath` or `toRepoPosixRelative` from `scripts/lib/path-classifier.js` (path-separator regressions are blocked by `scripts/__tests__/cross-platform-path-handling.test.js`).
- For any "is this path inside/outside directory X" decision, call `isPathInsideDirectory` / `isPathOutsideDirectory` (or the low-level `isOutsideRelative`) from `scripts/lib/path-classifier.js` -- never hand-roll `path.relative(dir, file).startsWith("..")`. On cross-drive Windows (repo on `D:\`, `os.tmpdir()` on `C:\`) `path.relative` returns an ABSOLUTE `C:\...` target, so a bare `startsWith("..")` mislabels an out-of-tree path as inside (and `!startsWith("..")` mislabels it as outside); the helpers add the required `path.isAbsolute` branch and symlink-resolve + case-fold on Windows. This is CATEGORY A of `scripts/__tests__/path-containment-policy.test.js`.
- Never root a `check-eol.js` / `fix-eol.js` test fixture under `os.tmpdir()`. `check-eol.js` drops any target whose ABSOLUTE path contains an excluded directory segment (its `excludeRegexes`: `.git`, `node_modules`, `Library`, `obj`, `Temp`, `Samples~`, `.vs`, `.venv`, `.artifacts`, `site`) BEFORE collecting text files. The `Temp` rule (`/(^|[\/\\])Temp([\/\\]|$)/`) is case-SENSITIVE and matches the capitalized `Temp` segment Windows `os.tmpdir()` always carries (`C:\Users\<u>\AppData\Local\Temp\...`), so on Windows EVERY `os.tmpdir()` fixture is excluded: the checker prints "EOL check skipped", exits 0, and a "dirty corpus must fail" / "must pass" precondition passes vacuously (Linux `/tmp` has no `Temp` segment, so it only surfaces on Windows). This exclusion is the DOMINANT mechanism; bare-vs-absolute targets and `git init`-ing the fixture dir do NOT remedy it (none of them changes the resolved, excluded path -- a `git init` "hermeticity" recipe was tried and is wrong, so there is NO git-init escape hatch). Sound fix: root the fixture in an IN-REPO, NON-excluded scratch dir (`fs.mkdtempSync(path.join(REPO_ROOT, "dxm-...-"))`, prefix gitignored), assert admissibility via the exported `isPathExcluded(dir)` from `check-eol.js`, and assert the checker did NOT print "EOL check skipped" before trusting exit status. This is CATEGORY B of `scripts/__tests__/path-containment-policy.test.js`; see [Jest Hook Robustness](./skills/scripting/jest-hook-robustness.md).
- When editing any `.yml`/`.yaml` file (including `.github/workflows/**` and `.github/actions/**`), run `npm run check:yaml` (and `npm run validate:workflows` / `npm run check:workflow-cspell` for workflows); preflight runs these for you -- see [YAML Line-Length Budget](./skills/github-actions/yaml-line-length.md).
- For zero-touch YAML line-length recovery, run the auto-fixers `npm run format:yaml:comments` (wraps long `#` comments) and `npm run format:yaml:lines` (rewrites long PowerShell strings in pwsh `run:` blocks); for anything else, shorten or externalize the script per [YAML Line-Length Budget](./skills/github-actions/yaml-line-length.md). When editing `.pre-commit-config.yaml` or hook scripts, the performance budget test (`scripts/__tests__/hook-perf-budget.test.js`) must stay green; see [Git Hook Performance Budget](./skills/performance/git-hook-performance.md).
...
- Run parser hook suite exactly as pre-push executes it: `node scripts/ensure-pre-commit.js run --hook-stage pre-push script-parser-tests --all-files`
...
- Full-repo spellcheck before pre-push: `npm run check:cspell:all`
...
- `UNITY_ACCELERATOR_ENDPOINT` is optional. When set, `scripts/unity/run-ci-tests.ps1` normalizes the value at the script boundary via `ConvertTo-NormalizedAcceleratorEndpoint` -- both bare `host:port` (for example `127.0.0.1:10080`) and `scheme://host:port[/path]` URL forms (for example `http://accelerator.example.com:10080/`) are accepted; userinfo, path, query, and fragment are stripped, IPv6 literals stay bracketed (`[::1]:10080`), and the normalized form is passed to Unity's `-cacheServerEndpoint` flag. An explicit port is required (the Unity CLI does not infer one). Both the original env value and the derived host:port are registered with `::add-mask::` before any downstream log line could echo them.
...
- For managed Jest tooling edits, run `node scripts/run-managed-jest.js --runTestsByPath scripts/__tests__/run-managed-jest.test.js scripts/__tests__/verify-managed-jest-fallback.test.js scripts/__tests__/validate-node-tooling.test.js` and then `node scripts/ensure-pre-commit.js run --hook-stage pre-push script-tests --all-files`.
...
- The targeted regression step in `.github/workflows/cross-platform-preflight.yml` may list tests as bash continued args or as a PowerShell `$tests = @(...)` array splatted into `--runTestsByPath @tests`. Keep parsing logic in `scripts/lib/cross-platform-preflight-gate.js`; `validate:workflows` must fail if that step becomes unparseable so native pre-push is not the first signal.
...
- Tests that assert Unity Windows loader missing-DLL diagnostics must not assume a native Windows host lacks VC++ or KnownDLL entries. Use `DXM_UNITY_FAKE_MISSING_IMPORTS` or synthetic non-resolving names when asserting the `MISSING DLL(s):` branch; `DXM_UNITY_FAKE_IMPORTS` alone only controls the import list and still resolves against the real host loader search path.
...
- Validators must check EFFECTIVE behavior, not a config-string proxy. For npm packaging, assert the file appears in the real `npm pack --json --dry-run` output (`scripts/validate-npm-meta.js`), never that a specific glob literal is present in package.json `files`: a broader glob such as `Editor/**` satisfies the intent yet fails a literal-string check. The fragile literal analyzer-DLL check was removed from `scripts/validate-workflows.js`; analyzer packaging is validated only by the tarball-based `validate-npm-meta`.
...
- For parser-script failures, verify both isolated and hook-parity execution before concluding root cause: run the focused Jest path first, then run `node scripts/ensure-pre-commit.js run --hook-stage pre-push script-parser-tests --all-files` from the same shell used for commit operations.
...
- [Change-Aware Preflight](./skills/scripting/change-aware-preflight.md)
