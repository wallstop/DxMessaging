...
- Known GitHub Actions dispatcher bug: self-hosted runners can report Online/Idle while `runner_id` stays at 0 for 7+ minutes, leaving a queued job stuck even when label sets match (see [Community Discussion #186811](https://github.com/orgs/community/discussions/186811)). The recovery path is `.github/workflows/stuck-job-watchdog.yml`, which audits queued runs every 10 minutes and reruns up to twice per 24h when an idle runner satisfies the queued job's labels. Manual recourse: re-run the failed job via the GitHub UI.
