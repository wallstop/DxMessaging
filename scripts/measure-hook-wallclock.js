...
function runPreCommit(stage, relPath) {
    ...
    const result = spawnSync(
        "pre-commit",
        ["run", "--hook-stage", stage, "--files", relPath],
        {
            cwd: REPO_ROOT,
            encoding: "utf8",
...
function formatHuman(results) {
    const out = [];
    out.push("Wall-clock measurement (Linux per-scenario budgets, see scripts/measure-hook-wallclock.js for rationale):");
    out.push("");
    for (const r of results) {
        const elapsed =
            r.elapsedMs === null ? "n/a" : `${r.elapsedMs.toString().padStart(6, " ")} ms`;
