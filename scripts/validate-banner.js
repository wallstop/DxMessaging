...
function checkPackageVersion(version, svgFile, svgText) {
    ...
    const badgeText = svgText.match(
        /<text[^>]*>v(\d+\.\d+\.\d+[^<]*)<\/text>\s*<\/g>/,
    );
    if (badgeText === null) {
        errs.push(
            makeError(
...
function calculateRepositoryTestCount(root) {
    return getRepositoryTestFiles(root).reduce(
        (sum, filePath) =>
            sum + countTestMarkers(filePath, fs.readFileSync(filePath, "utf8")),
        0,
    );
}
...
function checkNoExternalResources(svgFile, svgText) {
    ...
    const imageHref = svgText.match(
        /<image\b[^>]*\s(?:xlink:)?href\s*=\s*"([^"]+)"/i,
    );
    if (imageHref !== null) {
        errs.push(
            makeError(
    ...
    const externalRef = svgText.match(
        /\b(?:xlink:)?href\s*=\s*"((?:https?:)?\/\/[^"]+)"/i,
    );
    if (externalRef !== null) {
        errs.push(
            makeError(
    ...
    // <style> with @import.
    const styleImport = svgText.match(
        /<style\b[^>]*>[\s\S]*?@import[\s\S]*?<\/style>/i,
    );
    if (styleImport !== null) {
        errs.push(
            makeError(
    ...
    if (/\bjavascript\s*:/i.test(svgText)) {
        errs.push(
            makeError(
                "javascript",
                svgFile,
                'literal "javascript:" found',
                "remove the javascript: URL",
            ),
        );
    }
    return errs;
...
function checkStatBadges(svgFile, svgText) {
    ...
    while ((m = badgeRe.exec(columnInner)) !== null) {
        ...
        if (!Number.isFinite(rectWidth) || rectWidth < minWidth) {
            const labelMatch = textElement.match(
                /<tspan[^>]*>([^<]*)<\/tspan>\s*<\/text>/,
            );
...
function checkTransformedBadgeLayout(svgFile, svgText) {
    ...
    if (rightColumnMatch !== null) {
        ...
        if (outerTranslate === null) {
            ...
            const badgeRe =
                /<g\b([^>]*)>\s*<rect\b([^>]*)\/>\s*<text\b[\s\S]*?<\/text>\s*<\/g>/g;
            let badgeIndex = 0;
            let m;
...
function checkUnusedDefs(svgFile, svgText) {
    ...
    const outsideDefs =
        svgText.slice(0, defsMatch.index) +
        svgText.slice(defsMatch.index + defsMatch[0].length);
        ...
        if (!ref.test(outsideDefs)) {
            warns.push(
                makeWarn(
                    "unused-defs",
                    svgFile,
                    `<defs> id="${id}" is never referenced via url(#${id})`,
                ),
            );
        }
    }
