...
function reportResults(applicable, results) {
    ...
    if (totalViolations === 0) {
        emit(`run-staged-validators: 0 violations (${filesNote}).`, false);
    } else {
        emit(
            `run-staged-validators: ${totalViolations} violation(s) across ${VALIDATORS.length} validator(s) (${filesNote}).`,
            true,
        );
    }
