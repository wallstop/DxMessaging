...
const SKILL_DOC_POLICY_DIR = path.join(
    ROOT_DIR,
    ".llm",
    "skills",
    "documentation",
    ...
    matchLine(line, ctx) {
        ...
        while ((match = this.pattern.exec(line)) !== null) {
            ...
            findings.push({
                ...
                term,
                suggestion:
                    suggestionForTerm(MARKETING_REPLACEMENTS, term) ||
...
RULES.push({
    ...
    severity: "error",
    description:
        "LLM-signature filler idiom; rewrite with concrete language.",
    ...
    matchLine(line) {
        ...
        while ((match = this.pattern.exec(line)) !== null) {
            ...
            findings.push({
                ...
                term,
                suggestion:
                    suggestionForTerm(FILLER_REPLACEMENTS, term) ||
                    "Rewrite without the filler idiom.",
            });
        }
        return findings;
    },
...
RULES.push({
    ...
    severity: "error",
    description:
        "Hedge or weak transition at sentence start; lead with the fact.",
...
const ALLOW_NEXT_RE =
    /<!--\s*prose-allow-next-line:\s*([A-Za-z0-9,'\- \t]*?)\s*-->/i;
const ALLOW_FILE_RE =
    /<!--\s*prose-allow-file:\s*([A-Za-z0-9,'\- \t]*?)\s*-->/gi;
...
const MALFORMED_MARKER_RE =
    /<!--\s*prose-allow(?:-next-line|-file)?:[^>]*$/i;
...
function stripAllowMarkerComments(line) {
    return line
        .replace(ALLOW_LINE_RE, "")
        .replace(ALLOW_NEXT_RE, "")
        .replace(ALLOW_FILE_RE, "");
}
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        ...
        if (isFenceOpenClose(rawLine)) {
            ...
            if (!inFence) {
                ...
                const residue = fenceMatch
                    ? rawLine.slice(fenceMatch[0].length)
                    : "";
                if (residue.trim() !== "") {
                    // Synthesize a virtual line whose column offsets match
                    // the original by left-padding with spaces.
                    const padded =
                        " ".repeat(rawLine.length - residue.length) + residue;
                    scanLine(
                        padded,
                        lineIndex,
                        violations,
        ...
        const sameLineAllow = sameLineMatch
            ? new Set(parseAllowList(sameLineMatch[1]))
            : null;
        ...
        for (const rule of RULES) {
            ...
            for (const f of findings) {
                ...
                if (activeNextLineAllow && activeNextLineAllow.has(termLower))
                    continue;
                violations.push({
                    file: filePath,
                    line: lineIndex + 1,
...
function formatBaselineEntry(violation) {
    const rel =
        path.relative(ROOT_DIR, violation.file) || violation.file;
...
function main() {
    ...
    if (args.baseline) {
        if (fs.existsSync(args.baseline)) {
            ...
            process.stderr.write(
                `validate-docs-prose: baseline file not found: ${args.baseline}\n`,
            );
            return 1;
        }
    }
    ...
    if (args.writeBaseline) {
        ...
        const sorted = allViolations
            .map(formatBaselineEntry)
            .sort();
    ...
    const filtered = baselineKeys.size > 0
    ...
    if (args.summary) {
        ...
        if (filtered.length === 0) {
            process.stdout.write(
                `validate-docs-prose: 0 violations across ${files.length} file(s).\n`,
            );
